...
function showLogin() { state.user = null; state.current = null; state.cases = []; state.tools = null; state.audit = null; state.tab = "overview"; $("#case-list").replaceChildren(); $("#main-content").replaceChildren(); $("#account").textContent = ""; $("#modal").close(); $("#modal-content").replaceChildren(); $("#workspace").hidden = true; $("#login-view").hidden = false; sessionStorage.removeItem("tfir-csrf"); }
async function loadWorkspace() {
  ...
  if (state.cases.length) await selectCase(state.cases.some((c) => c.id === state.current?.id) ? state.current.id : state.cases[0].id); else { state.current = null; render(); }
}
...
function render() {
  ...
  $("#main-content").innerHTML = `<section class="case-heading"><div><p class="eyebrow">CASE ${esc(c.id.slice(0,8))}</p><h1>${esc(c.title)}</h1><div class="subline">${badge(c.closed ? "closed" : "investigating", c.closed ? "" : "green")}<span>Opened ${esc(time(c.created))}</span></div></div><div class="button-row"><button data-action="member">Manage access</button><button class="primary" data-action="proposal">+ Propose operation</button></div></section>
    <div class="stats"><div class="stat"><span>EVIDENCE SOURCES</span><strong>${c.artifacts.filter((a) => a.kind === "source").length}</strong><small>Original bytes preserved</small></div><div class="stat"><span>PENDING DECISIONS</span><strong>${pending}</strong><small>Human review required</small></div><div class="stat"><span>FINDING RECORDS</span><strong>${c.findings.length}</strong><small>Claims and review history</small></div><div class="stat"><span>AUDIT RECORDS</span><strong>${c.integrity.event_count}</strong><small>Signed local checkpoint</small></div></div>
...
function findingsView() {
  const renderFinding = (f, index, candidate) => `<article class="finding"><div class="pill-list">${badge(f.classification, "blue")}${badge(candidate ? "candidate · unreviewed" : (state.current.challenge_resolutions.some((r) => r.disposition === "finding_withdrawn" && state.current.challenges.some((c) => c.id === r.challenge_id && c.finding_id === f.id)) ? "withdrawn after review" : state.current.challenges.some((c) => c.finding_id === f.id) ? "accepted · see challenge history" : "human accepted"), candidate ? "amber" : "green")}${f.severity ? badge(f.severity) : ""}</div><h3>${esc(f.claim)}</h3><div class="meta">${esc((f.citations || []).map((x) => typeof x === "string" ? x : `${x.chunk_id} (${x.relation})`).join(" · "))}</div>${f.limitations ? `<p class="muted">${esc(f.limitations)}</p>` : ""}${candidate ? `<div class="button-row"><button class="compact" data-action="finding" data-id="${index}">Propose acceptance</button></div>` : `<div class="button-row"><button class="compact" data-action="challenge" data-id="${esc(f.id)}">Challenge finding</button></div>`}</article>`;
  const candidates = allCandidates();
  return `<section class="panel"><div class="panel-head"><h2>Accepted findings</h2><small>HUMAN REVIEW DOES NOT CHANGE EVIDENCE TYPE</small></div>${state.current.findings.length ? state.current.findings.map((f,i) => renderFinding(f,i,false)).join("") : empty("No accepted findings", "A tool result remains a candidate until a separate human-reviewed acceptance decision.")}</section>${challengesView()}<section class="panel"><div class="panel-head"><h2>Candidate findings</h2><button class="compact" data-action="suggest">Propose analysis</button></div>${candidates.length ? candidates.map((f,i) => renderFinding(f,i,true)).join("") : empty("No candidates yet", "Approve a rule analysis, a research pack run, or a configured model assessment.")}</section>`;
}
function challengesView() {
  return `<section class="panel"><div class="panel-head"><h2>Finding challenges</h2></div>${state.current.challenges.length ? state.current.challenges.map((c) => { const r = state.current.challenge_resolutions.find((r) => r.challenge_id === c.id); return `<article class="finding">${badge(r ? r.disposition : "open challenge", "amber")}<h3>${esc(c.explanation)}</h3><p class="meta">Finding ${esc(c.finding_id)}</p><details><summary>Challenge and review history</summary><pre>${pretty({challenge:c,resolution:r || null})}</pre></details>${r ? `<p>${esc(r.explanation)}</p>` : `<button data-action="resolve-challenge" data-id="${esc(c.id)}">Propose challenge resolution</button>`}</article>`; }).join("") : empty("No finding challenges", "New evidence can challenge an accepted finding. The original claim and review history stay available.")}</section>`;
}
function auditView() {
  if (!state.audit) return empty("Loading audit records", "Retrieving the signed case ledger.");
  return `<section class="panel"><div class="panel-head"><h2>Accountability trail</h2><button class="compact" data-action="refresh">↻ Refresh</button></div><div class="panel-body"><p class="meta">CHECKPOINT ${esc(state.audit.checkpoint.body.head_hash)}</p><p class="muted">Every record includes an actor, timestamp, predecessor hash, and operation metadata. Sensitive evidence and model transcripts are held separately.</p></div>${[...state.audit.events].reverse().map((e) => `<article class="audit-row"><span class="audit-seq">${esc(e.body.sequence.padStart(3,"0"))}</span><div><h3>${esc(e.body.event)}</h3><span class="meta">${esc(e.body.actor)} · ${esc(e.hash.slice(0,24))}…</span><details><summary>Inspect record</summary><pre>${pretty(e)}</pre></details></div><time>${esc(time(e.body.timestamp))}</time></article>`).join("")}${state.audit.page.has_more ? `<div class="panel-body"><button data-action="audit-older">Load older audit records</button></div>` : ""}</section>`;
}
function packsView() {
...
function modal(title, body, footer = '<button type="button" data-action="close-modal">Cancel</button><button class="primary" type="submit">Save</button>', handler = null) {
  ...
  $("#modal-form").addEventListener("submit", async (event) => {
    ...
    try { await handler(new FormData(event.target), event.submitter?.value); $("#modal").close(); if (state.user) await refresh(); }
    catch (error) { const node = $("#modal-error"); if (node) node.textContent = error.message; else notify(error.message, true); }
...
function defaults(tool) {
  ...
  const values = {
    reconcile_execution:{execution_id:"",artifact_ids:ids,assessment:"unable_to_determine",conclusion:"",residual_uncertainty:""},
    challenge_finding:{finding_id:"",explanation:"",citations:[]},
    resolve_challenge:{challenge_id:"",disposition:"uncertainty_retained",explanation:"",citations:[]},
...
function argumentFields(tool, initial) {
  ...
  const html = Object.entries(properties).map(([name,schema]) => {
    ...
    else if (["execution_id","finding_id","challenge_id"].includes(name)) {
      const choices = name === "execution_id" ? state.current.executions.filter((e) => e.state === "OUTCOME_UNKNOWN" && !state.current.reconciliations.some((r) => r.execution_id === e.id)) : name === "finding_id" ? state.current.findings : state.current.challenges.filter((c) => !state.current.challenge_resolutions.some((r) => r.challenge_id === c.id));
      widget = `<select name="arg-${name}" required><option value="">Select the exact record</option>${choices.map((r) => `<option value="${esc(r.id)}" ${r.id === value ? "selected" : ""}>${esc(r.claim || r.explanation || r.id)}</option>`).join("")}</select>`;
    }
...
async function dispatch(action, id) {
  ...
  if (action === "reconcile") return newProposal("reconcile_execution",{execution_id:id});
  if (action === "challenge") { const finding = state.current.findings.find((f) => f.id === id); return newProposal("challenge_finding",{finding_id:id,citations:finding.citations.map((c) => ({...c,relation:"contradicts"}))}); }
  if (action === "resolve-challenge") { const challenge = state.current.challenges.find((c) => c.id === id); return newProposal("resolve_challenge",{challenge_id:id,citations:challenge.citations}); }
  if (action === "audit-older") { const p = state.audit.page; const older = await api(`/api/cases/${state.current.id}/audit?limit=100&before=${p.next_before}&through=${p.through}`); state.audit = {...older,events:[...older.events,...state.audit.events]}; render(); return; }
  if (action === "password") return modal("Change password", '<label>Current password<input name="current_password" type="password" autocomplete="current-password" required></label><label>New password<input name="new_password" type="password" autocomplete="new-password" minlength="14" maxlength="256" required></label><label>Confirm new password<input name="confirm_password" type="password" autocomplete="new-password" minlength="14" maxlength="256" required></label><p class="muted">This signs you out of every session and invalidates your pending approvals. Historical reviews remain preserved.</p>', undefined, async (form) => { if (form.get("new_password") !== form.get("confirm_password")) throw new Error("Passwords differ"); await api("/api/password",{method:"POST",body:{current_password:form.get("current_password"),new_password:form.get("new_password")}}); showLogin(); notify("Password changed. Sign in again."); });

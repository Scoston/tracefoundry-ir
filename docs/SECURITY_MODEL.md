...
The application code, operating system, local administrator, protected state directory, signing keys, and configured provider destination are trusted in version 0.1. A host administrator controlling keys, state, and code can impersonate users or rewrite history. The design's independent approval service, external witness, and managed keys are required to reduce that trust concentration.
...
| Revoked or stale authority | Current roles, membership, lifetime, policy, code digest, and revocation audit checked again at use | Revocation racing an external dispatch cannot retract the disclosure |

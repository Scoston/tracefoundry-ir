...
Version 0.1 provides a runnable local application with the controls below. The original design describes a broader enterprise system. This table is the authoritative implementation boundary.

| Area | Implemented now | Remaining work |
| --- | --- | --- |
| Human authority | Named local accounts, password reauthentication, immutable proposal digest, signed reviews, current roles and membership, expiry, revocation | Enterprise IdP, MFA/step-up attestations, identity lifecycle synchronization, service identities |
...
| Execution | Unique durable intent, serialized local execution, no automatic retry, recorded failed/unknown state | Credential broker, workload identity, transactional outbox workers, enterprise queue, reconciliation-resolution action |
| Findings | Candidates separated from accepted claims; citations required; observation/hypothesis/unresolved/contradicted labels preserved | Automated contradiction linking and mature analyst challenge workflow |
...
| UI | Login, case list, membership, staging, proposals, review, execution/results, findings, packs, audit trail | Accessibility certification, large-case pagination, production-scale performance qualification |
...
| Packaging | Python package, dependency lock files, Dockerfile, source schemas, CI, tests, operating documents | Organizational release approval, penetration testing, hosted deployment, disaster recovery exercise |
